/**
 * A more lenient rate limiter for dashboard requests
 * This middleware limits the frequency of dashboard requests to prevent abuse
 */
...
/**
 * Dashboard rate limiter middleware
...
function dashboardRateLimiter(req, res, next) {
  ...
  if (requestCounts[ip].count > MAX_REQUESTS_PER_WINDOW) {
    console.warn(`Dashboard rate limit exceeded for IP: ${ip}`);
